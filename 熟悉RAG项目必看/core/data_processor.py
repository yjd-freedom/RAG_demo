...
try:
    from core.embedding_processor import ChineseClipImageEmbedder
    from core.milvus_manager import MilvusImageManager  # 使用简化版
except ImportError as e:
    print(f"导入本地模块失败: {e}")
    raise
...
class DataProcessor:
    def __init__(self, data_root: Optional[str] = None):
        ...
        Args:
            data_root (str, optional):
                手办数据根目录路径。若未提供，则使用 config/config.py 中的默认路径。
                此参数用于支持 FastAPI 接口传入自定义路径。
        """
        # 1. 获取模型路径和模型名
        ...
        # 支持外部传入 data_root，否则回退到配置文件中的默认路径
    ...
    def process_all_attractions(self) -> Dict:
        ...
        # 使用 self.data_root（可能来自外部传入，也可能来自配置）
