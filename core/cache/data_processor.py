...
class DataProcessor:
    def __init__(self):
        ...
        hf_model_name = "yjd-freedom/chinese-clip"

        # 2. 检查是否需要从 Hugging Face 下载
        ...
        if not (has_config and has_weight):
            logger.info(f"本地模型不完整或不存在，正在从 Hugging Face 下载: {hf_model_name}")
            logger.info(f"将保存到: {model_path}")
